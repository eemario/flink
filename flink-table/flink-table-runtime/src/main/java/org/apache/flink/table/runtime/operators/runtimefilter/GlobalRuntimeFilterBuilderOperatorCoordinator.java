...
/**
 * The operator coordinator for {@link GlobalRuntimeFilterBuilderOperator}. The coordinator collects
 * {@link RuntimeFilteringEvent} then redistributes to listening source coordinators.
 */
public class GlobalRuntimeFilterBuilderOperatorCoordinator
        implements OperatorCoordinator, CoordinationRequestHandler {
    private static final Logger LOG =
            LoggerFactory.getLogger(GlobalRuntimeFilterBuilderOperatorCoordinator.class);

    private final CoordinatorStore coordinatorStore;
    private final List<String> runtimeFilteringDataListenerIDs;

    private RuntimeFilteringData receivedFilteringData;

    public GlobalRuntimeFilterBuilderOperatorCoordinator(
            Context context, List<String> runtimeFilteringDataListenerIDs) {
        this.coordinatorStore = checkNotNull(context.getCoordinatorStore());
        this.runtimeFilteringDataListenerIDs = checkNotNull(runtimeFilteringDataListenerIDs);
    }
    ...
    public void handleEventFromOperator(int subtask, int attemptNumber, OperatorEvent event) {
        RuntimeFilteringData currentData =
                ((RuntimeFilteringEvent) ((SourceEventWrapper) event).getSourceEvent()).getData();
        if (receivedFilteringData == null) {
            ...
            if (RuntimeFilteringData.isEqual(receivedFilteringData, currentData)) {
                ...
                throw new IllegalStateException(
                        "RuntimeFilteringData is recomputed but not equal. "
                                + "Triggering global failover in case the result is incorrect. "
                                + " It's recommended to re-run the job with runtime filter disabled.");
            }
        }

        for (String listenerID : runtimeFilteringDataListenerIDs) {
            coordinatorStore.compute(
                    ...
                    (key, oldValue) -> {
                        ...
                        if (oldValue == null || oldValue instanceof OperatorEvent) {
                            ...
                            // Runtime filtering event is expected to be sent only once. So after
    ...
    /** Provider for {@link GlobalRuntimeFilterBuilderOperatorCoordinator}. */
    public static class Provider extends RecreateOnResetOperatorCoordinator.Provider {
        private final List<String> runtimeFilteringDataListenerIDs;

        public Provider(OperatorID operatorID, List<String> runtimeFilteringDataListenerIDs) {
            super(operatorID);
            this.runtimeFilteringDataListenerIDs = checkNotNull(runtimeFilteringDataListenerIDs);
        }
        ...
        protected OperatorCoordinator getCoordinator(Context context) throws Exception {
            return new GlobalRuntimeFilterBuilderOperatorCoordinator(
                    context, runtimeFilteringDataListenerIDs);
        }
    }
}

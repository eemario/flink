...
import org.apache.flink.table.planner.plan.rules.{FlinkBatchRuleSets, FlinkIncrementalRuleSets, FlinkStreamRuleSets}

import org.apache.calcite.plan.hep.HepMatchOrder

/** Defines a sequence of programs to optimize flink incremental table plan. */
object FlinkIncrementalProgram {
  ...
  def buildProgram(tableConfig: ReadableConfig): FlinkChainedProgram[BatchOptimizeContext] = {
    ...
    chainedProgram.addLast(
      PHYSICAL,
      FlinkVolcanoProgramBuilder.newBuilder
        .add(FlinkIncrementalRuleSets.PHYSICAL_OPT_RULES)
    ...
    chainedProgram.addLast(
      ...
      FlinkGroupProgramBuilder
        .newBuilder[BatchOptimizeContext]
        .addProgram(new FlinkIncrementalChangelogModeInferenceProgram, "Changelog mode inference")
        .addProgram(
          FlinkHepRuleSetProgramBuilder.newBuilder
            .setHepRulesExecutionType(HEP_RULES_EXECUTION_TYPE.RULE_SEQUENCE)
            .setHepMatchOrder(HepMatchOrder.BOTTOM_UP)
            .add(FlinkIncrementalRuleSets.PHYSICAL_REWRITE)
            .build(),
          "physical rewrite"
        )
        .build()

 ...
 */
package org.apache.flink.table.planner.plan.rules.physical.incremental
...
/**
 * Rule that matches [[StreamPhysicalGroupAggregate]] on [[CommonPhysicalExchange]] with the
 * following condition:
 *   1. two-phase aggregation is enabled in given TableConfig, 2. all aggregate functions are
 *      mergeable, 3. the input of exchange does not satisfy the shuffle distribution,
 *
...
class TwoStageOptimizedAggregateRule
  extends RelOptRule(
    operand(
      ...
      operand(classOf[CommonPhysicalExchange], operand(classOf[RelNode], any))),
  ...
  override def matches(call: RelOptRuleCall): Boolean = {
    ...
    isTwoPhaseEnabled && matchesTwoStage(call.rel(0), call.rel(2))
  }

  def matchesTwoStage(agg: StreamPhysicalGroupAggregate, realInput: RelNode): Boolean = {
    val needRetraction = !ChangelogPlanUtils.isInsertOnly(realInput.asInstanceOf[FlinkPhysicalRel])
  ...
  override def onMatch(call: RelOptRuleCall): Unit = {
    ...
    val newInput =
      satisfyDistribution(FlinkConventions.BATCH_PHYSICAL, localHashAgg, globalDistribution)

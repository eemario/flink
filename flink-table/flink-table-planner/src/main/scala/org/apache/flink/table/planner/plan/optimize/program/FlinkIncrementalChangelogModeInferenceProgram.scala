...
import org.apache.flink.table.planner.plan.`trait`.{ModifyKind, ModifyKindSet, ModifyKindSetTrait, ModifyKindSetTraitDef, UpdateKind, UpdateKindTrait, UpdateKindTraitDef}
...
import org.apache.flink.table.planner.plan.nodes.physical.batch.{BatchPhysicalCorrelate, BatchPhysicalJoinBase}
import org.apache.flink.table.planner.plan.nodes.physical.common.{CommonPhysicalExchange, CommonPhysicalTableSourceScan}
import org.apache.flink.table.planner.plan.nodes.physical.stream.{StreamPhysicalCalcBase, StreamPhysicalChangelogNormalize, StreamPhysicalCorrelateBase, StreamPhysicalDataStreamScan, StreamPhysicalDeduplicate, StreamPhysicalDropUpdateBefore, StreamPhysicalExchange, StreamPhysicalExpand, StreamPhysicalGroupAggregate, StreamPhysicalGroupTableAggregate, StreamPhysicalGroupTableAggregateBase, StreamPhysicalGroupWindowAggregateBase, StreamPhysicalIntermediateTableScan, StreamPhysicalIntervalJoin, StreamPhysicalJoin, StreamPhysicalLegacySink, StreamPhysicalLegacyTableSourceScan, StreamPhysicalLimit, StreamPhysicalLookupJoin, StreamPhysicalMatch, StreamPhysicalMiniBatchAssigner, StreamPhysicalOverAggregate, StreamPhysicalPythonGroupAggregate, StreamPhysicalPythonGroupTableAggregate, StreamPhysicalPythonOverAggregate, StreamPhysicalRank, StreamPhysicalSink, StreamPhysicalSort, StreamPhysicalSortLimit, StreamPhysicalTableSourceScan, StreamPhysicalTemporalJoin, StreamPhysicalTemporalSort, StreamPhysicalUnion, StreamPhysicalValues, StreamPhysicalWatermarkAssigner, StreamPhysicalWindowAggregate, StreamPhysicalWindowDeduplicate, StreamPhysicalWindowJoin, StreamPhysicalWindowRank, StreamPhysicalWindowTableFunction}
import org.apache.flink.table.planner.plan.utils.{ChangelogPlanUtils, FlinkRelOptUtil, RankProcessStrategy}
...
import org.apache.calcite.rel.core.{Union, Values}
import org.apache.calcite.util.ImmutableBitSet

import scala.collection.JavaConversions._

/** An optimize program to infer ChangelogMode for every physical node. */
class FlinkIncrementalChangelogModeInferenceProgram
  extends FlinkOptimizeProgram[BatchOptimizeContext] {
  override def optimize(root: RelNode, context: BatchOptimizeContext): RelNode = {
    // step1: satisfy ModifyKindSet trait
    val physicalRoot = root.asInstanceOf[FlinkPhysicalRel]
  ...
  private class SatisfyModifyKindSetTraitVisitor {
     ...
     */
    def visit(
        rel: FlinkPhysicalRel,
        requiredTrait: ModifyKindSetTrait,
        requester: String): FlinkPhysicalRel = rel match {
      case sink: Sink =>
        ...
        sink.copy(sinkTrait, children).asInstanceOf[FlinkPhysicalRel]
      ...
      case join: BatchPhysicalJoinBase =>
        // batch join support insert only
        val children = visitChildren(rel, ModifyKindSetTrait.INSERT_ONLY)
        val providedTrait = ModifyKindSetTrait.INSERT_ONLY
        createNewNode(join, children, providedTrait, requiredTrait, requester)
      ...
      case _: CommonCalc | _: StreamPhysicalCorrelateBase | _: BatchPhysicalCorrelate |
          _: StreamPhysicalLookupJoin | _: CommonPhysicalExchange | _: StreamPhysicalExpand |
      ...
      case ts: CommonPhysicalTableSourceScan =>
      ...
      case _: StreamPhysicalDataStreamScan | _: StreamPhysicalLegacyTableSourceScan | _: Values =>
    ...
    private def visitChildren(
        parent: FlinkPhysicalRel,
        requiredChildrenTrait: ModifyKindSetTrait): List[FlinkPhysicalRel] = {
    ...
    private def visitChildren(
        ...
        requester: String): List[FlinkPhysicalRel] = {
    ...
    private def visitChild(
        ...
        requester: String): FlinkPhysicalRel = {
      val child = parent.getInput(childOrdinal).asInstanceOf[FlinkPhysicalRel]
      this.visit(child, requiredChildTrait, requester)
    }

    private def getNodeName(rel: FlinkPhysicalRel): String = {
      val prefix = "Exec"
    ...
    private def deriveQueryDefaultChangelogMode(queryNode: RelNode, name: String): ChangelogMode = {
      val newNode =
        visit(queryNode.asInstanceOf[FlinkPhysicalRel], ModifyKindSetTrait.ALL_CHANGES, name)
      getModifyKindSet(newNode).toChangelogMode
    }

    private def createNewNode(
        node: FlinkPhysicalRel,
        children: List[FlinkPhysicalRel],
        providedTrait: ModifyKindSetTrait,
        requiredTrait: ModifyKindSetTrait,
        requestedOwner: String): FlinkPhysicalRel = {
      if (!providedTrait.satisfies(requiredTrait)) {
        ...
        val tempNode = node.copy(node.getTraitSet, children).asInstanceOf[FlinkPhysicalRel]
      ...
      node.copy(newTraitSet, children).asInstanceOf[FlinkPhysicalRel]
    }
  }
   ...
   */
  private class SatisfyUpdateKindTraitVisitor(private val context: BatchOptimizeContext) {
     ...
     */
    def visit(rel: FlinkPhysicalRel, requiredTrait: UpdateKindTrait): Option[FlinkPhysicalRel] =
      rel match {
        case sink: Sink =>
          val sinkRequiredTraits = inferSinkRequiredTraits(sink)
          // TODO: support upsert materialize?
        ...
        case join: StreamPhysicalJoin =>
          ...
          val children = join.getInputs.zipWithIndex.map {
            case (child, childOrdinal) =>
              val physicalChild = child.asInstanceOf[FlinkPhysicalRel]
        ...
        case join: BatchPhysicalJoinBase =>
          // batch join support insert only
          createNewNode(join, visitChildren(join, UpdateKindTrait.NONE), requiredTrait)

        case temporalJoin: StreamPhysicalTemporalJoin =>
          val left = temporalJoin.getLeft.asInstanceOf[FlinkPhysicalRel]
          val right = temporalJoin.getRight.asInstanceOf[FlinkPhysicalRel]
        ...
        case _: StreamPhysicalCorrelateBase | _: BatchPhysicalCorrelate |
            _: StreamPhysicalLookupJoin | _: CommonPhysicalExchange | _: StreamPhysicalExpand |
            _: StreamPhysicalMiniBatchAssigner | _: StreamPhysicalWatermarkAssigner |
            _: StreamPhysicalWindowTableFunction =>
          // transparent forward requiredTrait to children
        ...
        case normalize: StreamPhysicalChangelogNormalize =>
          ...
          if (
            ...
            if (ManagedTableListener.isManagedTable(catalog, catalogTable)) {
              ...
              val inputPhysicalRel = input.asInstanceOf[FlinkPhysicalRel]
    ...
    private def visitChildren(
        parent: FlinkPhysicalRel,
        requiredChildrenTrait: UpdateKindTrait): Option[List[FlinkPhysicalRel]] = {
      val newChildren = for (child <- parent.getInputs) yield {
        this.visit(child.asInstanceOf[FlinkPhysicalRel], requiredChildrenTrait) match {
          case None =>
    ...
    private def createNewNode(
        node: FlinkPhysicalRel,
        childrenOption: Option[List[FlinkPhysicalRel]],
        providedTrait: UpdateKindTrait): Option[FlinkPhysicalRel] = childrenOption match {
      case None =>
        None
      case Some(children) =>
        ...
        val newTraitSet = node.getTraitSet.plus(providedTrait)
        Some(node.copy(newTraitSet, children).asInstanceOf[FlinkPhysicalRel])
    }
    ...
    private def visitRankStrategies(
        ...
        applyRankStrategy: RankProcessStrategy => FlinkPhysicalRel): Option[FlinkPhysicalRel] = {
    ...
    private def visitSink(
        sink: FlinkPhysicalRel,
        sinkRequiredTraits: Seq[UpdateKindTrait]): Option[FlinkPhysicalRel] = {
      val children = sinkRequiredTraits.flatMap(t => visitChildren(sink, t))
      if (children.isEmpty) {
        None
      } else {
        val sinkTrait = sink.getTraitSet.plus(UpdateKindTrait.NONE)
        Some(sink.copy(sinkTrait, children.head).asInstanceOf[FlinkPhysicalRel])
      }
    }
     ...
     */
    private def inferSinkRequiredTraits(sink: Sink): Seq[UpdateKindTrait] = {
      val childModifyKindSet = getModifyKindSet(sink.getInput)
      val onlyAfter = onlyAfterOrNone(childModifyKindSet)
      val beforeAndAfter = beforeAfterOrNone(childModifyKindSet)
     ...
     */
    private def analyzeUpsertMaterializeStrategy(sink: Sink): Boolean = {
      val tableConfig = unwrapTableConfig(sink)
      val inputChangelogMode =
        ChangelogPlanUtils.getChangelogMode(sink.getInput.asInstanceOf[FlinkPhysicalRel]).get
